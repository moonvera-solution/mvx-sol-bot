...
// import { formatAmmKeysToApi } from './raydium-utils/formatAmmKeys';
// import { formatClmmKeys } from './raydium-utils/formatClmmKeys';
import {
  buildAndSendTx,
...
// export async function routeSwap(input: TestTxInputInfo) {
...
//     await Clmm.fetchMultiplePoolInfos({ connection, poolKeys: clmmPools, chainTime: new Date().getTime() / 1000 })).map((i) => i.state)
...
//   const txV = new VersionedTransaction(messageSetUp);

//   sendJitoBundleFromIx("swap route jito bundle: ", [txV]);
...
async function howToUse() {
  ...
  const outputToken = DEFAULT_TOKEN.SOLFI
