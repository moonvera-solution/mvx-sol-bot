...
import { getSolanaDetails } from "../api/priceFeeds/birdEye";
...
export async function display_single_spl_positions(ctx: any, isRefresh: boolean) {
    const chatId = ctx.chat.id;
    const userWallet = ctx.session.portfolio.wallets[ctx.session.activeWalletIndex]?.publicKey;
    const userPosition: any = await UserPositions.find({ positionChatId: chatId, walletId: userWallet },  { positions: { $slice: -7} } );
    const connection = new Connection(`${ctx.session.env.tritonRPC}${ctx.session.env.tritonToken}`);
    ...
    const [solprice, positionPool] = await Promise.all([
        getSolanaDetails(),
        synchronizePools(userPosition[0].positions, ctx)
    ]);
    
    // ctx.session.solPrice = solPrice; // Assuming you want to store the solPrice in the session
    ctx.session.positionPool = positionPool;
    let currentIndex = ctx.session.positionIndex;
    if(userPosition[0].positions[currentIndex]){
        currentIndex = 0; 
