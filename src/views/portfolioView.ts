...
export async function display_spl_positions(ctx: any) {
    ...
    const userPosition: any = await UserPositions.find({ positionChatId: ctx.chat.id, walletId: userWallet }, { positions: { $slice: -7 } })
    const userPoolKeys = ctx.session.positionPool;

    const connection = new Connection(`${ctx.session.env.tritonRPC}${ctx.session.env.tritonToken}`);

    const solprice = await getSolanaDetails();
    if (userPoolKeys.length == 0) {
        await ctx.api.sendMessage(ctx.chat.id, "No positions found.", { parse_mode: 'HTML' });
        return;
    }
    let currentIndex = ctx.session.positionIndex;
    ctx.session.activeTradingPool = userPoolKeys[currentIndex];
    ...
    try {
        let fullMessage = '';
        // console.log("out loop userPosition:  ",userPosition)
        // console.log("out loop poolKeys:  ",poolKeys)

        for (let index in userPosition[0].positions) {
            let pos = userPosition[0].positions[index];

            const poolKeys = userPoolKeys[index];
            console.log("index", index, poolKeys.baseMint)

            const token = String(poolKeys.baseMint);
            ...
            const tokenInfo = await quoteToken({
                baseVault: poolKeys.baseVault, quoteVault: poolKeys.quoteVault,
                baseDecimals: poolKeys.baseDecimals, quoteDecimals: poolKeys.quoteDecimals, baseSupply: poolKeys.baseMint, connection
            });
...
export async function display_single_spl_positions(ctx: any) {
    ...
    ctx.session.positionPool = userPosition[0].positions;
...
=======
            ...
            function poolKeysExists(poolKeysArray: any, newPoolKeys: any) {
                return poolKeysArray.some((existingKeys: any) => existingKeys.id === newPoolKeys.id)
            }
>>>>>>> Stashed changes
...
=======
            let pos = userPosition[0].positions[currentIndex];
            let poolKeys = ctx.session.positionPool[currentIndex];
...
export async function syncDisplayPositions(ctx: any) {
    const userWallet = ctx.session.portfolio.wallets[ctx.session.activeWalletIndex]?.publicKey;
    const userPosition: any = await UserPositions.find({ positionChatId: ctx.chat.id, walletId: userWallet }, { positions: { $slice: -7 } })
    ctx.session.positionPool = [];
    if (userPosition && userPosition[0]?.positions) {
        for (let index in userPosition[0].positions) {

            const pos = userPosition[0].positions[index];
            const poolKeys = await getRayPoolKeys(ctx, pos.baseMint);
            ctx.session.positionPool.push(poolKeys);
