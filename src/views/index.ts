...
export async function display_token_details(ctx: any, isRefresh: boolean) {
  ...
  const [
    ...
    runAll(ctx, raydiumId),
  ]);
  const lowpriorityFees = priorityFees[0];
  const mediumpriorityFees = priorityFees[1];
  const highpriorityFees = priorityFees[2];
  const maxpriorityFees = priorityFees[3];
